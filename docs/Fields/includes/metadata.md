...
> **Scope:** Currently only **kit / participant colors** (`PARTICIPANT_COLORS`) are returned. Other raw metadata stored against a fixture (e.g. neutral venue, kickoff side, formations, attendance) is not yet exposed via this include.
...
| `type_id`        | integer | The external type ID. `348` = `PARTICIPANT_COLORS`.                                                                                   |
| `developer_name` | string  | The constant-style identifier for the metadata type.                                                                                  |
| `value`          | integer | Always `null` for `PARTICIPANT_COLORS`. Present for template consistency with other type-based includes (see `meta` on [Metrics](#)). |
| `meta`           | object  | Context-specific payload. For `PARTICIPANT_COLORS`, contains `kit` and `participant`. See below.                                      |
